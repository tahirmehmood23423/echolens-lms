...
function openState(hash = location.hash) {
  ...
  } else if (['courses','free','paid','quests'].includes(parts[0])) {
  ...
  } else if (['home','compare','events','profile','announcements','feedback'].includes(parts[0])) state = {v:parts[0] === 'compare' ? 'home' : parts[0]};
...
async function restoreNav(state, revision = NAV_REVISION) {
  ...
  if (['home','events','announcements','feedback'].includes(state.v)) openTab(state.v);
...
(async () => {
  ...
  if (ME) loadCerts();
  await catReady;
  const initialState = history.state?.v && openRoute(history.state) === (location.hash || '#home') ? history.state : (openState() || {v:'home'});
...
function showSignup() {
  $('signupArea').innerHTML = `
    <form id="suForm" style="text-align:left;border-top:1px solid var(--line);padding-top:14px">
      ...
      <fieldset style="border:1px solid var(--line);border-radius:12px;padding:12px 14px;margin:0 0 4px">
        <legend class="s" style="padding:0 6px;color:var(--ink);font-weight:600">Your age <span style="color:var(--danger)">*</span></legend>
        <label class="field" style="margin-bottom:6px"><span><input name="age_declaration" type="radio" value="adult" style="width:auto" required> I am 18 or older</span></label>
        <label class="field" style="margin-bottom:0"><span><input name="age_declaration" type="radio" value="minor" style="width:auto" required> I am under 18 and have my parent or guardian&rsquo;s permission to enroll</span></label>
      </fieldset>
      <p class="hint">EchoLens is open to school and university learners. We record your answer with your account; we do not contact your parent or guardian to check it. Accounts declared under 18 are kept out of public profiles, the Talent Marketplace and recruiter contact.</p>
  ...
  f.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const out = await api('/api/auth/register-open', {
        method: 'POST',
        body: JSON.stringify({ name: f.name.value, email: f.email.value.trim(), whatsapp: f.whatsapp.value, city: f.city.value.trim(), university: f.university.value.trim(), degree: f.degree.value.trim(), study_year: f.study_year.value, goal: f.goal.value.trim(), marketing_opt_in: f.marketing_opt_in.checked, age_declaration: (f.querySelector('input[name="age_declaration"]:checked') || {}).value, code: f.code ? f.code.value.trim() : undefined }),
      });
      if (out.password) {
...
function openTab(tab) {
  if (tab === 'compare') tab = 'home';
  EL.drafts.flushAll();
  if (['home','courses','events','profile','announcements','feedback'].includes(tab)) OPEN_COURSE_REQUEST++;
  if (['home','events','profile','announcements','feedback'].includes(tab)) pushNav({v:tab});
  ['home', 'courses', 'course', 'solve', 'events', 'eventDetail', 'announcements', 'profile', 'feedback'].forEach((t) => {
