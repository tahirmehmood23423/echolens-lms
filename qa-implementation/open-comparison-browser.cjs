...
(async () => {
  ...
  for (const width of [1440,390]) {
    ...
    await page.goto(base+'/open');await settle();assert.ok(await page.locator('#tab-compare').isVisible());
    await page.locator('#cmpTabFree').focus();await page.keyboard.press('ArrowRight');assert.equal(await page.locator('#cmpTabPaid').getAttribute('aria-selected'),'true');
    assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    await page.screenshot({path:path.join(evidence,'comparison-completed-'+width+'.png'),fullPage:true});
    await page.locator('#cmpCardPaid .cmp-cta a').click();assert.equal(await page.locator('#cFree').inputValue(),'paid');
    ...
    await page.goto(base+'/open#free');await settle();await page.locator('.oc-lang-btn').filter({hasText:'JavaScript'}).click();const family=page.url();
    ...
    await page.getByRole('link',{name:'Compare free and paid courses'}).click();await settle();assert.ok(await page.locator('#tab-compare').isVisible());
