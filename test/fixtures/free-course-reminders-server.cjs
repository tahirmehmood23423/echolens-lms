...
Object.assign(process.env, { NODE_ENV: 'test', DATABASE_URL: '', DIRECT_URL: '', PORT: '0', DB_PATH: path.join(runtime, 'store.json'), UPLOAD_DIR: path.join(runtime, 'uploads'), ENROLLMENT_HOLD_HOURS: '0', JWT_SECRET: 'synthetic-admissions-test-secret', MAIL_DRY_RUN: 'true' });
const store = require('../../store');
const roles = ['admin', 'student_coordinator', 'finance', 'instructor', 'student', 'hr', 'coordinator'];
const users = roles.map((role, i) => ({ id: i + 1, name: 'Synthetic ' + role, username: role, role, email: role + '@qa.invalid', profile: { phone: '03001234567', city: 'Lahore', university: 'Sample', degree: 'BS CS', study_year: '3' }, onboarding_complete: true }));
...
let reminderService, now = new Date();
const reminderModule = require('../../free-course-reminders');
const create = reminderModule.createFreeCourseReminders;
reminderModule.createFreeCourseReminders = (s, m, options) => reminderService = create(s, m, { ...options, clock: () => now, pause: async () => {} });
