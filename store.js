...
let lastFlushTiming = null;
function flushHealth() {
  return {
    disabled: process.env.FLUSH_DISABLED === 'true',
    pendingRecordCount: require('./normalized-flush-plan').pendingCounts(data, lastPersistedSnapshot || {}),
    lastFlushDurationMs: lastFlushTiming?.totalMs ?? null,
    lastFlushTiming,
 ...
 * functions is synchronous internally (verified: no `await` appears
 ...
 */
async function persistAllToPostgresNormalized(snapshot, captureMs = 0) {
  if (process.env.FLUSH_DISABLED === 'true') {
    console.warn('[flush] disabled by env - skipping; pending writes are not durable');
    return { skipped: true };
  }
  const { prepareFlush, failedOperation } = require('./normalized-flush-plan');
  const { getPrismaClient, getQueryCount } = require('./prisma-client');
  const prisma = getPrismaClient();
  const prevSnapshot = lastPersistedSnapshot || {};
  const started = performance.now();
  const queryStart = getQueryCount();
  let plan, lastOp = null, preparationMs = 0, dbMs = 0;
  try {
    plan = prepareFlush(snapshot, prevSnapshot);
    preparationMs = performance.now() - started;
    const dbStarted = performance.now();
    try {
      if (plan.operations.length) {
        // Step 2 keeps the existing whole-flush atomic boundary. Step 3 will
        // group logical units before packing them into chunk transactions.
        await prisma.$transaction(async (tx) => {
          for (const operation of plan.operations) await operation.execute(tx);
        }, { maxWait: 10000, timeout: 60000 });
      }
    } finally { dbMs = performance.now() - dbStarted; }
  } catch (err) {
    ...
    // anything new.
    lastOp = failedOperation(err);
    preparationMs = preparationMs || performance.now() - started;
    recordFlushTiming(false);
  ...
  lastPersistedSnapshot = plan.nextSnapshot;
  lastSuccessfulFlushAt = new Date().toISOString();
  consecutiveFlushFailures = 0;
  recordFlushTiming(true);

  function recordFlushTiming(ok) {
    lastFlushTiming = {
      ok, captureMs, preparationMs, scanSerializeMs: captureMs + preparationMs, dbMs,
      totalMs: captureMs + performance.now() - started,
      queries: getQueryCount() - queryStart, operations: plan?.operations.length || 0,
      rowsWritten: ok ? plan.rowsWritten : {},
    };
    console.log('[flush] ' + JSON.stringify(lastFlushTiming));
  }
}

/** Capture only when this serialized queue entry starts, after the caller's
 * synchronous logical operation has completed. Later live mutations are
 * isolated from this frozen snapshot and remain pending for the next flush.
 */
function queuePersistToPostgres() {
  const thisOp = pgQueueTail.then(() => {
    if (process.env.FLUSH_DISABLED === 'true') return persistAllToPostgresNormalized(null);
    const started = performance.now();
    const snapshot = require('./normalized-flush-plan').captureSnapshot(data);
    return persistAllToPostgresNormalized(snapshot, performance.now() - started);
  });
  pgQueueTail = thisOp.catch(() => {});
  return thisOp;
}
...
const OFFICIAL_CATALOGUE = [
  ...
  { code: 'SC-09', title: 'Freelancing & Client Acquisition Mastery', tier: 'Short Course', weeks: 6, hours: 24, price_pkr: 10000, badges: ['new'], summary: 'Positioning, proposals, pricing and client communication - turning skills into income.' },
  ...
  { code: 'ST-02', title: 'Graphic Designing', tier: 'Short Course', weeks: 6, hours: 24, price_pkr: 12000, badges: [], summary: 'A full professional design track: brand identity, collateral and a portfolio built to client standards.' },
  { code: 'ST-03', title: 'UI/UX Designing', tier: 'Short Course', weeks: 6, hours: 24, price_pkr: 12000, badges: ['high_demand'], summary: 'Research, wireframing and high-fidelity product design in Figma, closing with a shipped-quality case study.' },
  ...
  // course codes.
  { code: 'FC-01', title: 'CS-101: Fundamentals of C', tier: 'Short Course', weeks: 5, hours: 12, price_pkr: 0, badges: ['free'], free_mode: 'signin', family: 'c', family_order: 1, summary: 'The language everything else was built on - compilation, memory, operators, branching and loops through to modular functions, with a real-life analogy, a video, and a scenario-based Compiler Quest for every one of 16 topics across 5 modules.' },
  { code: 'FC-02', title: 'CS-102: Fundamentals of C++ (with OOP)', tier: 'Short Course', weeks: 4, hours: 12, price_pkr: 0, badges: ['free'], free_mode: 'signin', family: 'cpp', family_order: 1, summary: 'Streams and iostream, modern types (auto/const), std::string and std::vector, references and overloading, through to encapsulated classes with constructors, destructors and validated getters/setters.' },
  { code: 'CS-104', title: 'CS-104: Fundamentals of Python', tier: 'Short Course', weeks: 5, hours: 12, price_pkr: 0, badges: ['free'], free_mode: 'signin', family: 'python', family_order: 1, summary: 'The bytecode interpreter and dynamic typing through strings, control flow and core data structures to functions, classes and safe file I/O, with a real-life analogy, a video, and a scenario-based Compiler Quest for every one of 15 topics across 5 modules.' },
  { code: 'CS-105', title: 'CS-105: Fundamentals of JavaScript', tier: 'Short Course', weeks: 5, hours: 12, price_pkr: 0, badges: ['free', 'new'], free_mode: 'signin', family: 'javascript', family_order: 1, summary: 'The JavaScript engine and console.log() through scoping, type coercion, arrays and objects to closures, currying and asynchronous code with Promises and async/await, with a real-life analogy, a video, and a scenario-based Compiler Quest for every one of 15 topics across 5 modules.' },
  { code: 'CS-106', title: 'CS-106: Fundamentals of HTML & CSS', tier: 'Short Course', weeks: 5, hours: 12, price_pkr: 0, badges: ['free', 'new'], free_mode: 'signin', family: 'web', family_order: 1, summary: 'HTML document structure, semantic markup, tables and forms, then CSS selectors, the box model, positioning, Flexbox and Grid and mobile-first responsive design, with a real-life analogy, a video, and a scenario-based Compiler Quest for every one of 15 topics across 5 modules.' },
  // Course 2 (Advanced) per language, completing each language's free
  // 2-course ladder: Course 1 (Fundamentals, the entry above) then this
  // one combined Advanced course, internally organised into a Basic tier
  // (levels 1-4) and an Advanced tier (levels 5-8) - see
  // tracks/curriculum-advanced-combined.js. Deliberately 2 courses per
  // language, not 3: an earlier version of this catalogue had 3 (with
  // separate Intermediate/Advanced course codes CS1.2/CS1.3 etc.), and
  // was collapsed to this 2-course shape per an explicit product decision
  // - no content was removed, it was reorganised into tiers within one
  // course instead.
  { code: 'CS1-ADV', title: 'Advanced C Programming', tier: 'Short Course', weeks: 4, hours: 12, price_pkr: 0, badges: ['free'], free_mode: 'signin', family: 'c', family_order: 2, summary: 'Systems & Memory Control: pointers, pointer arithmetic and double pointers, the heap (malloc/realloc/free), structs and linked lists, bitwise hardware masks, and file I/O through to function-pointer dispatch - four modules, one Compiler Quest per topic.' },
  { code: 'CPP2-ADV', title: 'Advanced C++ Programming', tier: 'Short Course', weeks: 4, hours: 12, price_pkr: 0, badges: ['free'], free_mode: 'signin', family: 'cpp', family_order: 2, summary: 'OOP & Architecture: inheritance, virtual dispatch and abstract interfaces, operator overloading and copy control (Rule of Three/Five), templates and STL algorithms with lambdas, and smart pointers with move semantics.' },
