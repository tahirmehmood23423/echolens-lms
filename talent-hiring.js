...
module.exports = {
  register(app, { authRequired, requireRecruiter, adminRequired, APP_URL, UPLOAD_DIR }) {
    ...
    app.get('/api/talent/recruiter/contact-requests', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
    ...
    app.get('/api/talent/recruiter/contact-requests/:id', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
    ...
    app.get('/api/talent/recruiter/contact-requests/:id/resume', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
      const r = await getRequestForUser(req.params.id, req.user.id);
    ...
    app.get('/api/talent/me/contact-requests', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
    ...
    app.post('/api/talent/me/contact-requests/:id/accept', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
      const r = await getRequestForUser(req.params.id, req.user.id);
    ...
    app.post('/api/talent/me/contact-requests/:id/decline', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
      const r = await getRequestForUser(req.params.id, req.user.id);
      if (!r || r.student_id !== req.user.id) return res.status(404).json({ error: 'Not found.' });
      if (r.status !== 'pending') return res.status(400).json({ error: 'This request has already been responded to.' });
    ...
    app.post('/api/talent/me/block-company', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
    ...
    app.get('/api/talent/me/blocked-companies', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
    ...
    app.delete('/api/talent/me/blocked-companies/:companyId', authRequired, requireDb, requireStudent, asyncRoute(async (req, res) => {
      await db.query('DELETE FROM blocked_companies WHERE student_id = $1 AND company_id = $2', [req.user.id, req.params.companyId]);
      res.json({ ok: true });
    }));

    /* --------------------------------- messaging --------------------------------- */
    app.get('/api/talent/contact-requests/:id/messages', authRequired, requireDb, asyncRoute(async (req, res) => {
      const r = await getRequestForUser(req.params.id, req.user.id);
    ...
    app.post('/api/talent/contact-requests/:id/messages', authRequired, requireDb, asyncRoute(async (req, res) => {
      const r = await getRequestForUser(req.params.id, req.user.id);
      if (!r) return res.status(404).json({ error: 'Not found.' });
      if (r.status !== 'accepted') return res.status(403).json({ error: 'Messaging opens once the contact request is accepted.' });
      const { body } = req.body || {};
    ...
    /* ---------------------------------- shortlists ---------------------------------- */
    app.get('/api/talent/shortlists', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
    ...
    app.post('/api/talent/shortlists', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
      const { name } = req.body || {};
    ...
    app.delete('/api/talent/shortlists/:id', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
    ...
    app.post('/api/talent/shortlists/:id/candidates', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
    ...
    app.delete('/api/talent/shortlists/:id/candidates/:studentId', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
      const list = await ownShortlist(req, res); if (!list) return;
    ...
    app.get('/api/talent/shortlists/:id/export.csv', authRequired, requireDb, requireRecruiter, asyncRoute(async (req, res) => {
      const list = await ownShortlist(req, res); if (!list) return;
    ...
    /* ------------------------------ reports (Phase 6) ------------------------------ */
    app.post('/api/talent/reports', authRequired, requireDb, asyncRoute(async (req, res) => {
    ...
    app.get('/api/admin/talent/reports', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
    ...
    app.post('/api/admin/talent/reports/:id/resolve', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
      await db.query(`UPDATE reports SET status='resolved', resolved_at=now(), resolved_by=$1 WHERE id=$2`, [req.user.id, req.params.id]);
      res.json({ ok: true });
    }));
    app.post('/api/admin/talent/reports/:id/dismiss', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
    ...
    app.post('/api/admin/talent/profiles/:userId/unpublish', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
    ...
    app.post('/api/admin/talent/projects/:id/unpublish', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
      const { reason } = req.body || {};
      if (!reason || !String(reason).trim()) return res.status(400).json({ error: 'Enter a reason - the student is told why.' });
    ...
    /* ------------------------------- analytics (Phase 6) ------------------------------- */
    app.get('/api/admin/talent/analytics', authRequired, adminRequired, requireDb, asyncRoute(async (req, res) => {
