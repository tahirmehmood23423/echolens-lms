...
**Admissions challan reminders (September 2026):** generating a challan in Admissions enables follow-up emails by default at 7, 4, 3 and 1 day before its deadline, on the deadline, and one day afterward. The worker checks every 15 minutes from 09:00 Asia/Karachi, waits for acceptance of the original challan email, and stops once Finance verifies payment or enrollment is recorded. The Admissions **Email reminders** button shows dates/history, previews the extension email and pauses/enables the schedule. Existing challans stay off until individually enabled. The extension contact defaults to `0314148929` and `finance@echolens.digital`; optionally set `ADMISSIONS_EXTENSION_PHONE` or the existing `FINANCE_EMAIL`. Uses existing transactional mail configuration and the registration's JSON status field; no migration is needed. See [verification and delivery behavior](qa-implementation/admissions-reminders.md).

**Client demo (`/demo`):** the existing website now includes read-only accounts for all 12 portal views, with short usernames and password `admin`. Share [DEMO_ACCOUNTS.md](DEMO_ACCOUNTS.md). These accounts exist only in an isolated demo worker with synthetic records, separate sessions and server-enforced write restrictions. Deploy the code and install production dependencies; no production users, database changes or provider keys are needed for this feature. The worker starts on demand, binds only to loopback, and stops after 20 minutes idle. Its disposable JSON/SQL stores and files are created under the system temporary directory. `demo/talent-schema.tar.gz` contains only an empty Talent schema and skill vocabulary, built with the pinned PGlite version; regenerate with `npm run demo:schema` after changing that version or the Talent migrations. See [demo verification](qa-implementation/demo-accounts.md).

**Manual certificate override (September 2026):** admins can open a portal course's **Manage → Issue certificate** form and tick **Issue even if the student has not submitted or completed the track**. For a whole cohort, use **Manage → Issue certificates (whole course)** and untick **Only students who completed the full quest track**. Both forms require completion by default; instructors cannot bypass that check. Overrides require enrollment, preserve learner work and grades, and are recorded in the private audit log. Repeated issuance reuses the existing certificate without a duplicate notification. No migration or new configuration is needed. See [verification and screenshots](qa-implementation/certificate-override.md).
...
**The compiler grew up.** Full-screen workspace (sidebar untouched) with a Focus mode that hides the brief; language picker with **Python, JavaScript, TypeScript, C, C++, Java, Go, SQL and HTML/CSS/JS** (live web preview with console output); normal copy/paste and drag/drop editing; exact-line error highlighting; and **datasets attached to tasks** - a teacher uploads `sales.csv` once, and it is mounted into every student's Python filesystem automatically, so `pd.read_csv('sales.csv')` just works. Students can copy the file path in one click.
